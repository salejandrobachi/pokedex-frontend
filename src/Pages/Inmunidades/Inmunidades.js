...
import InmuneData from '../../Components/DisplayInmune';
import './Inmunidades.css';

const { Title } = Typography;

function Inmunidades() {
  ...
  const [current, setCurrent] = useState('inmunidad');
  const onClick = e => {
    setCurrent(e.key);
    switch (e.key) {
      case 'home':
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      setError(null);
      try {
        const initialData = await fetchApiData('inmunidad', 'GET');
        setData(initialData || []);
        setLoading(false);
      } catch (err) {
        setError(err);
  ...
  const FunFiltrar = (event) => {
    ...
    const endpoint = inputValue ? `inmunidad/nombre?name=${inputValue}` : 'inmunidad';
