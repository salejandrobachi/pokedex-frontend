...
import { Typography, Menu, Input, Image, Row, Col } from 'antd';
import { useLazyFetch } from '../../FetchApi/useFetch';
import ResData from '../../Components/DisplayRes';
...
function Resistencias() {
  ...
  const [current, setCurrent] = useState('resistencia');
  const onClick = e => {
    setCurrent(e.key);
    switch (e.key) {
      case 'home':
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      setError(null);
      try {
        const initialData = await fetchApiData('resistencia', 'GET');
        setData(initialData || []);
        setLoading(false);
      } catch (err) {
        setError(err);
  ...
  const FunFiltrar = (event) => {
    ...
    const endpoint = inputValue ? `resistencia/nombre?name=${inputValue}` : 'resistencia';
