...
import { Typography, Menu, Input, Col, Row, Image } from 'antd';
import { useLazyFetch } from '../../FetchApi/useFetch';
import TypeData from '../../Components/DisplayType';
import './Tipos.css';
...
function Tipos() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      setError(null);
      try {
        const initialData = await fetchApiData('tipos', 'GET');
        setData(initialData || []);
        setLoading(false);
      } catch (err) {
        setError(err);
  ...
  const FunFiltrar = (event) => {
    ...
    const endpoint = inputValue ? `tipos/nombre?name=${inputValue}` : 'tipos';
